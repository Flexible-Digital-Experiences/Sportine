/* ============================================================
   social.js  —  Feed Dinámico Oficial con Soporte Form-Data
============================================================ */
...
function renderFeed(listaPosts) {
  ...
  if (!listaPosts || listaPosts.length === 0) {
      feed.innerHTML = `
        <div style="text-align:center; padding: 60px 20px; display:flex; flex-direction:column; align-items:center;">
            ...
            <button onclick="window.location.href='buscar-amigo.html'" style="background: linear-gradient(135deg, #00A896, #028090); color: white; border: none; padding: 12px 28px; border-radius: 50px; font-weight: bold; font-family: 'Sora', sans-serif; cursor: pointer; box-shadow: 0 4px 10px rgba(0, 168, 150, 0.3); font-size: 0.9rem;">
                BUSCAR AMIGOS
            </button>
        </div>`;
      return;
  ...
  feed.innerHTML = listaPosts.map(function(post, i) {
    ...
    // HTML Opcional para la imagen
    let imagenHtml = '';
    // Tu backend devuelve post.imagen que podria ser base64 o URL 
    // Lo pintaremos si existe
    if(post.imagen) {
...
window.toggleLike = async function(btnContext, id, currentLikedStatus) {
    // 1. Obtener estado actual del DOM (porque puede haber cambiado si hace click rápido)
    ...
    try {
        if (newIsLiked) {
            await Api.darLike(id);
        } else {
            await Api.quitarLike(id);
        }
    } catch(err) {
        // En caso de fallo de red, se hace un "rollback" revertiendo la vista.
        console.error("Fallo de red devolviendo el like. Revirtiendo tarjeta...", err);
...
window.toggleComentarios = async function(postId) {
    ...
    // Si ya está abierto, lo ocultamos y listo.
    if (section.style.display !== 'none') {
        section.style.display = 'none';
        return;
    }
    
    // Mostrar cajón en estado de visualización inicial
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fileInput.addEventListener('change', (e) => {
      ...
      if(file) {
          ...
          // Mostrar vista previa leyendolo
  ...
  if (btnVerAmigos) btnVerAmigos.addEventListener('click', function() { window.location.href = 'mis-contactos.html'; });
  var btnNotif = document.getElementById('btn-notificaciones');
  if (btnNotif) btnNotif.addEventListener('click', function() { window.location.href = 'notificaciones.html'; });
  var btnAmigo = document.getElementById('btn-buscar-amigo');
  if (btnAmigo) btnAmigo.addEventListener('click', function() { window.location.href = 'buscar-amigo.html'; });

  // Acciones visuales nav/sidebar
